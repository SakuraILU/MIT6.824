...
func (rf *Raft) InstallSnapshot(args *InstallSnapshotArgs, reply *InstallSnapshotReply) {
	...
	if lastIncludedIndex != -1 {
		// 如果找到了，截断之前的Log
		DPrintf("[Raft %d {%d}] InstallSnapshot from %d, lastIncludedIndex: %d, commitIndex: %d", rf.me, rf.currentTerm, args.LeaderId, args.LastIncludedIndex, rf.commitIndex)
	...
	// 更新snapshot
	rf.persister.SaveStateAndSnapshot(rf.statePersistData(), args.Data)
